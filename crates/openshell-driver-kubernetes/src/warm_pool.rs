...
pub async fn annotate_claimed_pod(
    client: &Client,
    namespace: &str,
    claim_name: &str,
    sandbox_id: &str,
    ...
    let sandbox_cr_name = claim
        .data
        .get("status")
        .and_then(|s| s.get("sandbox"))
        .and_then(|s| s.get("name"))
        .and_then(|n| n.as_str())
        .ok_or_else(|| WarmPoolError::ClaimFailed("claim missing sandbox name in status".into()))?;
    ...
    pods.patch(
        sandbox_cr_name,
        &kube::api::PatchParams::apply("openshell-gateway"),
        &kube::api::Patch::Merge(&pod_patch),
    )
    .await?;

    let gvk = GroupVersionKind::gvk("agents.x-k8s.io", "v1beta1", "Sandbox");
    let resource = ApiResource::from_gvk(&gvk);
    let sandbox_api: Api<DynamicObject> = Api::namespaced_with(client.clone(), namespace, &resource);
    let cr_patch = json!({
        "metadata": {
            "labels": {
    ...
    sandbox_api.patch(
        sandbox_cr_name,
        &kube::api::PatchParams::apply("openshell-gateway"),
        &kube::api::Patch::Merge(&cr_patch),
    )
    .await?;

    info!(
        sandbox_cr = %sandbox_cr_name,
        sandbox_id = %sandbox_id,
        "Annotated warm pool pod and labeled Sandbox CR"
    );
    Ok(())
}
